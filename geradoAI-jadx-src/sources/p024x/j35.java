package p024x;

import android.content.Context;
import android.os.Build;
import android.util.Pair;
import com.unity3d.services.UnityAdsConstants;
import dalvik.system.DexClassLoader;
import java.io.Closeable;
import java.io.File;
import java.io.FileInputStream;
import java.io.FileOutputStream;
import java.io.IOException;
import java.lang.reflect.Method;
import java.nio.ByteBuffer;
import java.nio.charset.Charset;
import java.util.Arrays;
import java.util.HashMap;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutionException;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.Future;
import java.util.concurrent.TimeUnit;
import java.util.concurrent.TimeoutException;

/* JADX INFO: loaded from: classes.dex */
public final class j35 implements h35 {

    /* JADX INFO: renamed from: a */
    public final Context f9824a;

    /* JADX INFO: renamed from: b */
    public final ExecutorService f9825b;

    /* JADX INFO: renamed from: c */
    public final y25 f9826c;

    /* JADX INFO: renamed from: d */
    public final g35 f9827d;

    /* JADX INFO: renamed from: e */
    public final b75 f9828e;

    /* JADX INFO: renamed from: f */
    public final xb5 f9829f;

    /* JADX INFO: renamed from: g */
    public final HashMap f9830g = new HashMap();

    /* JADX INFO: renamed from: h */
    public final long f9831h;

    /* JADX INFO: renamed from: i */
    public final File f9832i;

    /* JADX INFO: renamed from: j */
    public boolean f9833j;

    /* JADX INFO: renamed from: k */
    public byte[] f9834k;

    /* JADX INFO: renamed from: l */
    public DexClassLoader f9835l;

    public j35(Context context, ExecutorService executorService, y25 y25Var, g35 g35Var, File file, b75 b75Var, long j, xb5 xb5Var) {
        this.f9824a = context;
        this.f9825b = executorService;
        this.f9826c = y25Var;
        this.f9827d = g35Var;
        this.f9828e = b75Var;
        this.f9829f = xb5Var;
        this.f9832i = new File(file, "rbp");
        this.f9831h = j;
    }

    /* JADX INFO: renamed from: c */
    public static void m5302c(File file) {
        if (file.exists()) {
            file.delete();
        }
    }

    /* JADX INFO: renamed from: d */
    public static void m5303d(Closeable closeable) {
        if (closeable != null) {
            try {
                closeable.close();
            } catch (Exception unused) {
            }
        }
    }

    @Override // p024x.h35
    /* JADX INFO: renamed from: a */
    public final Method mo4630a(String str, String str2) {
        Future future = (Future) this.f9830g.get(new Pair(str, str2));
        b75 b75Var = this.f9828e;
        if (future == null) {
            b75Var.m2421b(302);
            return null;
        }
        try {
            return (Method) future.get(this.f9831h, TimeUnit.MILLISECONDS);
        } catch (InterruptedException | ExecutionException unused) {
            b75Var.m2421b(304);
            return null;
        } catch (TimeoutException unused2) {
            b75Var.m2421b(303);
            return null;
        }
    }

    /* JADX INFO: renamed from: b */
    public final void m5304b(File file) throws Throwable {
        if (new File(file.toString().concat("/1773362577665.tmp")).exists()) {
            return;
        }
        File file2 = new File(file.toString().concat("/1773362577665.dex"));
        if (file2.exists()) {
            long length = file2.length();
            if (length > 0) {
                byte[] bArr = new byte[(int) length];
                FileInputStream fileInputStream = null;
                try {
                    try {
                        FileInputStream fileInputStream2 = new FileInputStream(file2);
                        try {
                            if (fileInputStream2.read(bArr) > 0) {
                                tf2 tf2VarM9128I = uf2.m9128I();
                                byte[] bytes = Build.VERSION.SDK.getBytes();
                                l06 l06Var = q06.f16308k;
                                l06 l06VarM7572t = q06.m7572t(bytes, 0, bytes.length);
                                tf2VarM9128I.m6370k();
                                ((uf2) tf2VarM9128I.f12060k).m9136M(l06VarM7572t);
                                byte[] bytes2 = "1773362577665".getBytes();
                                l06 l06VarM7572t2 = q06.m7572t(bytes2, 0, bytes2.length);
                                tf2VarM9128I.m6370k();
                                ((uf2) tf2VarM9128I.f12060k).m9135L(l06VarM7572t2);
                                throw new f35();
                            }
                            m5303d(fileInputStream2);
                        } catch (IOException e) {
                            e = e;
                            fileInputStream = fileInputStream2;
                            this.f9828e.m2423d(e, 301);
                            m5303d(fileInputStream);
                        } catch (f35 e2) {
                            e = e2;
                            fileInputStream = fileInputStream2;
                            this.f9828e.m2423d(e, 301);
                            m5303d(fileInputStream);
                        } catch (Throwable th) {
                            th = th;
                            fileInputStream = fileInputStream2;
                            m5303d(fileInputStream);
                            m5302c(file2);
                            throw th;
                        }
                    } catch (Throwable th2) {
                        th = th2;
                    }
                } catch (IOException e3) {
                    e = e3;
                } catch (f35 e4) {
                    e = e4;
                }
                m5302c(file2);
            }
        }
    }

    @Override // p024x.h35
    public final synchronized void zza() {
        FileInputStream fileInputStream;
        FileOutputStream fileOutputStream;
        a75 a75VarM2420a = this.f9828e.m2420a(201);
        try {
            try {
                try {
                    a75VarM2420a.m1863a();
                    try {
                        try {
                            byte[] bArrM2855a = c15.m2855a("iYhu9lTnEtAJiBwgX4ku/kdhSswKTa2KWLA7fmPqgg8=", false);
                            if (bArrM2855a.length != 32) {
                                throw new f35();
                            }
                            byte[] bArr = new byte[16];
                            ByteBuffer.wrap(bArrM2855a, 4, 16).get(bArr);
                            for (int i = 0; i < 16; i++) {
                                bArr[i] = (byte) (bArr[i] ^ 68);
                            }
                            this.f9834k = bArr;
                            try {
                                File file = this.f9832i;
                                file.mkdirs();
                                String string = file.toString();
                                StringBuilder sb = new StringBuilder(string.length() + 18);
                                sb.append(string);
                                sb.append(UnityAdsConstants.DefaultUrls.AD_ASSET_PATH);
                                sb.append("1773362577665");
                                sb.append(".jar");
                                File file2 = new File(sb.toString());
                                if (!file2.exists()) {
                                    g35 g35Var = this.f9827d;
                                    byte[] bArr2 = this.f9834k;
                                    g35Var.getClass();
                                    byte[] bArrM4353a = g35.m4353a("", bArr2);
                                    file2.createNewFile();
                                    FileOutputStream fileOutputStream2 = new FileOutputStream(file2);
                                    try {
                                        if (Build.VERSION.SDK_INT >= 34) {
                                            file2.setReadOnly();
                                        }
                                        fileOutputStream2.write(bArrM4353a, 0, bArrM4353a.length);
                                        fileOutputStream2.close();
                                    } catch (Throwable th) {
                                        try {
                                            fileOutputStream2.close();
                                        } catch (Throwable th2) {
                                            th.addSuppressed(th2);
                                        }
                                        throw th;
                                    }
                                }
                                File file3 = this.f9832i;
                                String string2 = file3.toString();
                                StringBuilder sb2 = new StringBuilder(string2.length() + 19);
                                sb2.append(string2);
                                sb2.append(UnityAdsConstants.DefaultUrls.AD_ASSET_PATH);
                                sb2.append("1773362577665");
                                sb2.append(".tmmp");
                                File file4 = new File(sb2.toString());
                                FileInputStream fileInputStream2 = null;
                                if (file4.exists()) {
                                    String string3 = file3.toString();
                                    StringBuilder sb3 = new StringBuilder(string3.length() + 18);
                                    sb3.append(string3);
                                    sb3.append(UnityAdsConstants.DefaultUrls.AD_ASSET_PATH);
                                    sb3.append("1773362577665");
                                    sb3.append(".dex");
                                    File file5 = new File(sb3.toString());
                                    if (!file5.exists()) {
                                        try {
                                            long length = file4.length();
                                            if (length <= 0) {
                                                m5302c(file4);
                                            } else {
                                                byte[] bArr3 = new byte[(int) length];
                                                fileInputStream = new FileInputStream(file4);
                                                try {
                                                    try {
                                                        if (fileInputStream.read(bArr3) <= 0) {
                                                            m5302c(file4);
                                                        } else {
                                                            c16 c16Var = c16.f4366b;
                                                            int i2 = e06.f5966a;
                                                            uf2 uf2VarM9127H = uf2.m9127H(bArr3, c16.f4367c);
                                                            if ("1773362577665".equals(new String(uf2VarM9127H.m9131F().m7575b())) && Arrays.equals(uf2VarM9127H.m9130E().m7575b(), this.f9826c.m10305c(uf2VarM9127H.m9129D().m7575b())) && Arrays.equals(uf2VarM9127H.m9132G().m7575b(), Build.VERSION.SDK.getBytes())) {
                                                                g35 g35Var2 = this.f9827d;
                                                                byte[] bArr4 = this.f9834k;
                                                                String str = new String(uf2VarM9127H.m9129D().m7575b());
                                                                g35Var2.getClass();
                                                                byte[] bArrM4353a2 = g35.m4353a(str, bArr4);
                                                                file5.createNewFile();
                                                                fileOutputStream = new FileOutputStream(file5);
                                                                try {
                                                                    fileOutputStream.write(bArrM4353a2, 0, bArrM4353a2.length);
                                                                    m5303d(fileInputStream);
                                                                    m5303d(fileOutputStream);
                                                                } catch (IOException | NullPointerException | f35 unused) {
                                                                    m5303d(fileInputStream);
                                                                    m5303d(fileOutputStream);
                                                                } catch (Throwable th3) {
                                                                    th = th3;
                                                                    fileInputStream2 = fileInputStream;
                                                                    m5303d(fileInputStream2);
                                                                    m5303d(fileOutputStream);
                                                                    throw th;
                                                                }
                                                            } else {
                                                                m5302c(file4);
                                                            }
                                                        }
                                                        m5303d(fileInputStream);
                                                    } catch (Throwable th4) {
                                                        th = th4;
                                                        fileOutputStream = null;
                                                    }
                                                } catch (IOException | NullPointerException | f35 unused2) {
                                                    fileOutputStream = null;
                                                }
                                            }
                                        } catch (IOException | NullPointerException | f35 unused3) {
                                            fileInputStream = null;
                                            fileOutputStream = null;
                                        } catch (Throwable th5) {
                                            th = th5;
                                            fileOutputStream = null;
                                        }
                                    }
                                }
                                try {
                                    this.f9835l = new DexClassLoader(file2.getAbsolutePath(), this.f9832i.getAbsolutePath(), null, this.f9824a.getClassLoader());
                                    m5302c(file2);
                                    File file6 = this.f9832i;
                                    m5304b(file6);
                                    m5302c(new File(String.format("%s/%s.dex", file6, "1773362577665")));
                                    for (final k35 k35Var : this.f9829f) {
                                        Pair pairCreate = Pair.create(k35Var.f10610a, k35Var.f10611b);
                                        HashMap map = this.f9830g;
                                        if (!map.containsKey(pairCreate)) {
                                            map.put(pairCreate, this.f9825b.submit(new Callable() { // from class: x.i35
                                                @Override // java.util.concurrent.Callable
                                                public final Object call() {
                                                    j35 j35Var = this.f9119a;
                                                    DexClassLoader dexClassLoader = j35Var.f9835l;
                                                    g35 g35Var3 = j35Var.f9827d;
                                                    byte[] bArr5 = j35Var.f9834k;
                                                    k35 k35Var2 = k35Var;
                                                    String str2 = k35Var2.f10610a;
                                                    String str3 = k35Var2.f10611b;
                                                    Class<?>[] clsArr = k35Var2.f10612c;
                                                    try {
                                                        g35Var3.getClass();
                                                        byte[] bArrM4353a3 = g35.m4353a(str2, bArr5);
                                                        Charset charset = g35.f7646a;
                                                        return dexClassLoader.loadClass(new String(bArrM4353a3, charset)).getMethod(new String(g35.m4353a(str3, bArr5), charset), clsArr);
                                                    } catch (ClassNotFoundException | NoSuchMethodException | NullPointerException | f35 e) {
                                                        throw new IllegalStateException(e);
                                                    }
                                                }
                                            }));
                                        }
                                    }
                                    this.f9833j = true;
                                    a75VarM2420a.m1865c();
                                } catch (Throwable th6) {
                                    m5302c(file2);
                                    File file7 = this.f9832i;
                                    m5304b(file7);
                                    m5302c(new File(String.format("%s/%s.dex", file7, "1773362577665")));
                                    throw th6;
                                }
                            } catch (IOException | NullPointerException | f35 e) {
                                throw new d15(e);
                            }
                        } catch (IllegalArgumentException e2) {
                            throw new f35(e2);
                        }
                    } catch (f35 e3) {
                        throw new d15(e3);
                    }
                } catch (Throwable th7) {
                    a75VarM2420a.m1864b(th7);
                    throw th7;
                }
            } catch (Throwable th8) {
                a75VarM2420a.m1865c();
                throw th8;
            }
        } catch (d15 e4) {
            a75VarM2420a.m1864b(e4);
        }
    }

    @Override // p024x.h35
    public final synchronized boolean zzb() {
        return this.f9833j;
    }
}
